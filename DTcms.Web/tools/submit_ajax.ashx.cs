...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            //获取传参信息===================================
            string hideGoodsJson = DTRequest.GetFormString("jsondata");//获取商品信息
            int payment_id = DTRequest.GetFormInt("payment_id");//支付方式
            string accept_name = Utils.ToHtml(DTRequest.GetFormString("accept_name"));//收货人姓名
            string province = Utils.ToHtml(DTRequest.GetFormString("province"));
            string city = Utils.ToHtml(DTRequest.GetFormString("city"));
            string area = Utils.ToHtml(DTRequest.GetFormString("area"));//所属省市区
            string address = Utils.ToHtml(DTRequest.GetFormString("address"));//收货地
            string mobile = Utils.ToHtml(DTRequest.GetFormString("mobile"));//手机
            string Wechat = Utils.ToHtml(DTRequest.GetFormString("Wechat"));//微信号
            string message = Utils.ToHtml(DTRequest.GetFormString("message"));//订单留言
            string Invitation_code = Utils.ToHtml(DTRequest.GetFormString("code"));//邀请码
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请输入正确网址！\"}");
                return;
            }
            //获取商品信息==================================
            List<Model.cart_keys> iList = (List<Model.cart_keys>)JsonHelper.JSONToObject<List<Model.cart_keys>>(hideGoodsJson);
            List<Model.cart_items> goodsList = ShopCart.ToList(iList, 0); //商品列表
            Model.cart_total goodsTotal = ShopCart.GetTotal(goodsList); //商品统计
            if (goodsList == null)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，商品为空，无法结算！\"}");
            ...
            //提交成功，返回URL
            context.Response.Write("{\"status\":1, \"url\":\"payment.aspx?id=" + result +"\", \"msg\":\"恭喜您，订单已成功提交！\"}");
            return;
        }
        #endregion

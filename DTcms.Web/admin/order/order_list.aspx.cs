...
namespace DTcms.Web.admin.order
{
    public partial class order_list : Web.UI.ManagePage
    {
        ...
        {
            Model.manager _manager = GetAdminInfo();
            ...
            this.roleId = _manager.role_id;//角色
            this.code = _manager.Code;//推官吗
            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_list", DTEnums.ActionEnum.View.ToString()); //检查权限
                RptBind("id>0" + CombSqlTxt(this.status, this.payment_status, this.express_status, this.keywords,this.roleId), "status asc,add_time desc,id desc");
            }
        }
        ...
        #region 组合SQL查询语句==========================
        protected string CombSqlTxt(int _status, int _payment_status, int _express_status, string _keywords, int roleId)
        {
